...
You have an `execute` tool that runs shell commands. Every response must use it — in this order:

1. Call `execute` with `scripts/search_docs.mjs "<query>"` — search before writing code
2. Write the code using the search results
3. Call `execute` with `scripts/validate.mjs --code '...' --model YOUR_MODEL_NAME --client-name YOUR_CLIENT_NAME --client-version YOUR_CLIENT_VERSION --artifact-id YOUR_ARTIFACT_ID --revision REVISION_NUMBER` — validate before returning
